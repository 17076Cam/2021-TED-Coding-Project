...
def save(self):
    global score
    global scoresave
    scoresave = [score]  # adds all the attributes together to make a person
    scoresave.append(score)  # adds the person to the database
    with open("scores.txt", "r+") as file:  # opens the text files where people are stored
        data = json.load(file)  # sets "data" as whats in the file
        data.append(score)  # adds the newest person onto that list
        file.seek(0)
        json.dump(data, file)  # overights everything ob txt with the old data + the new data
...
with app.frame("end_screen"):
    ...
    app.addButton("Save Score", save, colspan=1, row=7, column=0)

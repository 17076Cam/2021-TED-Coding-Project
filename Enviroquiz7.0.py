...
import json
"""
Enviromental quiz V7.0
make program look better and also improve commenting
"""
# variables
quiz_progress = -1
question_db = []
...
def press(button):
    ...
    if quiz_progress == 10:
        ...
        app.addLabel("Congratulations you finished", colspan=4, row=0, column=0)
...
def skip_question(self):
    ...
    if quiz_progress == 10:
        ...
        app.addLabel("You scored: %d" % score, colspan=4, row=1, column=0)
# this saves the data to a txt file in the same spot as the .py
def save(self):
    global score
    global scoresave
    scoresave = [score]
...
with app.frame("end_screen"):
    app.addLabel("If you wish to save score press button below", colspan=4, row=3, column=0)
